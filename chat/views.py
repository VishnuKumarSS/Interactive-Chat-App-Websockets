...
def upload_view(request, room_name):
    """Open and save images uploaded by users from mongo db"""
    data = {}
    if request.method == 'POST':
        img = request.FILES['img_name']
        img_title = request.POST['title_name']

        # read and save data in gridFS - file system
        img_binary = fs.put(img.read(), 
                            title=img_title,
...
def room(request, room_name):
    ...
    images = []
    check = fs.exists({'room_name': room_name}) # check whether roomname is there or not
    if check:
        for img in fs.find({'room_name': room_name}).sort("uploadDate", -1): # for file in file system
            data = {
...
# Backup mongo upload_view - oct 20
